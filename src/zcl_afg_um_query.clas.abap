...
*select po_document,
    CASE io_request->get_entity_id( ).
      WHEN 'ZRISHI_QUERY_SAMPLE'.

        "1. Response with count.
        ...
        IF io_request->is_data_requested(  ).
          ...
          "3. get filter as SQL string.
          DATA(lv_sql_filter) = io_request->get_filter(  )->get_as_sql_string(  ).

          "4. Paging to restricted number records to be sent to UI.
...
*                 from zrishi_podoc
*                 where po_desc like %lv_search% and po_doc = '100'
*                 order by  po_documnet
*                 INTO CORRESPONDING FIELDS OF TABLE @lt_purchase_docs
*                 offset 0 upto 20 rows.
